...
export function PortfolioPage() {
  ...
  return <div className="portfolio-shell">
    ...
    <main>
      <section id="home" className="hero-section hero">
        ...
        <div className="hero-copy hero-content" ref={heroCopyRef}>
          <div className="hero-status"><span className="status-dot-box"><span /></span><p className="status-text-box">Available for software engineering opportunities</p></div>
      ...
      <section id="lab" className="section lab-section">
        ...
        <div className="lab-grid">{labCards.map((card, index) => <article className="glass-panel lab-card" data-reveal key={card.label}><span className="card-index">0{index + 1}</span><h3>{card.label}</h3><p>{card.detail}</p><span className="lab-mark">â—</span></article>)}</div>
      </section>
    ...
    <footer><div><strong>Shivpal Rathod</strong><span>Python full-stack / AI + LLM integration</span></div><div><a href={links.github} {...external}>GitHub</a><a href={links.linkedin} {...external}>LinkedIn</a><a href={links.leetcode} {...external}>LeetCode</a><a href={links.email}>Email</a></div><p>Â© 2026 Shivpal Rathod</p></footer>
    <div className={`custom-cursor ${cursor.active ? "cursor-active" : ""} ${cursor.project ? "cursor-project" : ""}`} style={{ left: cursor.x, top: cursor.y }}><span>{cursor.project ? "VIEW â†’" : ""}</span></div>
